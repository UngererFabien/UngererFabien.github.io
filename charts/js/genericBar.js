...
d3.chart("genericBars", {
	initialize: function(){
		...
		this.TooltipVals = chart.layer('tooltipVals', this.tooltipVals, {
			dataBind: function(data){
				return this.selectAll('.tooltipVal').data(data, d => d.id);
			},
			insert: function(){
	...
	transform: function(data){
		...
		var totalValue = d3.sum(data, d => d.val);
	
		var underPercentage = (EzChartsConfig.aggregateUnder/100)*totalValue;

		var totalAggreg = d3.sum(function (d) {
			if(d.val < underPercentage){
				chart.aggregatedData.push(d);
			} else aggregData.push(d);
